...
test.describe('Auth page tests', () => {
  ...
  test('should disallow navigation when signed out', async ({
    page,
    baseURL,
  }) => {
    const pageInfo: PageInfo = { page, baseURL: baseURL || '' };
    const validatePageURLParams: URLValidationParams = {
      ...pageInfo,
      path: 'auth',
    };
    await isSignOut(pageInfo);
  ...
  test('should verify if auth nav link is active', async ({
    page,
    baseURL,
  }) => {
    const routerLinkOptions: RouterLinkParams = {
      page,
      innerText: 'authentification',
    };
